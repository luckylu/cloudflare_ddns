...
func main() {
	...
	if recordCreated {
		LoopUpdateRecord(config, recordId, schedule)
	} else {
		_, createResult := CreateRecord(config, ip)
		LoopUpdateRecord(config, createResult.Result.Id, schedule)
	}
}

func LoopUpdateRecord(config Config, recordId string, schedule *gocron.Scheduler) {
	schedule.Every(int(config.Interval)).Seconds().Do(UpdateRecord, config, recordId)
